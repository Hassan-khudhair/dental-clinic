                ...
                <tr key={`${p.id}-detail`}>
                  <td colSpan={6} className="bg-gray-50 px-6 py-4">
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <div>
                        <p className="mb-2 text-xs font-bold uppercase tracking-wider text-clinic-muted">الحجوزات</p>
                        {p.reservations?.length ? (
                          <div className="space-y-1.5">
                            {p.reservations.map((r) => (
                              <div key={r.id} className="flex items-center justify-between rounded-lg bg-white px-3 py-2 text-xs shadow-sm">
                      ...
                      <div>
                        <p className="mb-2 text-xs font-bold uppercase tracking-wider text-clinic-muted">العلاجات</p>
                        {p.treatments?.length ? (
                          <div className="space-y-1.5">
                            {p.treatments.map((t) => (
                              <div key={t.id} className="rounded-lg bg-white px-3 py-2 text-xs shadow-sm">
